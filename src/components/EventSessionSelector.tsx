...
import { Event, ConfEvent } from '@/features/broadcast/types';
                    ...
                    <MenubarContent className="bg-neutral-800 border-neutral-700 max-h-[300px] overflow-y-auto">
                        {futureEvents && futureEvents.map((evt) => (
                            <MenubarItem
                                ...
                                onClick={() => window.location.href = `/saas/broadcast/event/${evt.id_event}/admin/0`}
                            >
                ...
                <MenubarMenu>
                    ...
                    <MenubarContent className="bg-neutral-800 border-neutral-700 max-h-[300px] overflow-y-auto">
                        <MenubarItem
                            ...
                            onClick={() => window.location.href = `/saas/broadcast/event/${idEvent}/admin/0`}
                        >
                            Toutes les sessions
                        </MenubarItem>
                        {confEventListLight && confEventListLight.map((conf) => (
                            <MenubarItem
                                ...
                                className="text-xs text-white focus:bg-neutral-700 focus:text-white cursor-pointer flex flex-col items-start gap-1 py-2"
                                onClick={() => window.location.href = `/saas/broadcast/event/${idEvent}/admin/${conf.id_conf_event}`}
                            >
