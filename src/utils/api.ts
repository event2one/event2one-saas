...
export async function fetchInitialData(params: FetchParams) {
    ...
    if (idConfEvent === 0) {
        partenaireListUrl = `${API_URL}?action=getPartenairesLight&params= AND id_event=${idEvent} and afficher !='0'&exclude_fields=event,conf_event&order_by=ordre_affichage ASC`;
    } else {
        partenaireListUrl = `${API_URL}?action=getPartenairesLight&params= AND id_event=${idEvent} AND id_conf_event IN(${idConfEvent}) and afficher !='0'&exclude_fields=event,conf_event&order_by=ordre_affichage ASC`;
    }
