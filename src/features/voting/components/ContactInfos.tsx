...
import { ContactDatas, JuryEvent } from '../types';

interface ContactInfosProps {
    translationsForLanguage: any;
    juryEvent: JuryEvent;
    handleContactDatas: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void;
    contactDatas: ContactDatas;
    setContactDatas: (data: ContactDatas) => void;
    contactStatutList: any[];
    props?: any; // Legacy props if needed
}

const ContactInfos: React.FC<ContactInfosProps> = ({ translationsForLanguage, juryEvent, handleContactDatas, contactDatas, setContactDatas, contactStatutList }) => {
    ...
    const updateContactFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
        e.preventDefault();
        ...
        // Note: In React/TS, we can't easily store File object in state if it expects string, 
        // but here contactDatas seems to handle it or we might need to adjust types.
        // For now, we'll assume it's fine or we might need to cast.
    ...
    const updateContact = async (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        e.preventDefault();
        const field = e.target.name;
    ...
    useEffect(() => {
        ...
        getCountries();
    }, []); // Removed contactDatas dependency to avoid infinite loop if getCountries doesn't depend on it
                ...
                <div className="mb-6 flex flex-col sm:flex-row sm:items-center">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="mt-4 flex space-x-4">
                            ...
                            {contactDatas.photos && contactDatas.photos.tiny && contactDatas.photos.tiny !== '' && !newPhoto &&
                                <img src={contactDatas.photos.tiny} alt="Contact" className="object-cover w-24 h-24 rounded-full border border-gray-200" />
                            }
                            {(!contactDatas.photos || !contactDatas.photos.tiny) && !newPhoto &&
